  ...
  <div class="addEdit-block">
    ...
    >
      <el-row>
          ...
          <el-form-item class="input" label="订单编号" prop="dingdanbianhao">
              <el-input v-model="ruleForm.dingdanbianhao" 
                placeholder="订单编号" readonly></el-input>
                                    ...
                                    <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="菜品名称" prop="caipinmingcheng">
          <el-select  @change="caipinmingchengChange" v-model="ruleForm.caipinmingcheng" placeholder="请选择菜品名称">
            <el-option
                v-for="(item,index) in caipinmingchengOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.caipinmingcheng" label="菜品名称" prop="caipinmingcheng">
        ...
        <div v-else>
	      ...
	      <el-input v-model="ruleForm.pingfen"
                placeholder="评分" readonly></el-input>
                                    ...
                                    <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.tianjiatupian" label="添加图片" prop="tianjiatupian">
          <file-upload
          tip="点击上传添加图片"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.tianjiatupian?ruleForm.tianjiatupian:''"
          @change="tianjiatupianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.tianjiatupian" label="添加图片" prop="tianjiatupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.tianjiatupian.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
                                                                  <el-col :span="12">
...
export default {
  ...
  methods: {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.pingfen = true;
            continue;
	            	            ...
	            	            if(o=='tianjiatupian'){
            this.ruleForm.tianjiatupian = obj[o];
	    this.ro.tianjiatupian = true;
            continue;
          }
	            	            if(o=='pingjianeirong'){
          ...
          this.$message.error(data.msg);
              ...
              if (data && data.code === 0) {
                ...
                this.$message.error(data.msg);
                    ...
                    // 下二随
    caipinmingchengChange () {
      this.$http({
        url: `follow/caipinxinxi/caipinmingcheng?columnValue=`+ this.ruleForm.caipinmingcheng,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
                                                                                if(data.data.caipinfenlei){
                this.ruleForm.caipinfenlei = data.data.caipinfenlei
              }
                                                                                                                                                                                        } else {
          this.$message.error(data.msg);
        }
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `caipinpingjia/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.caipinpingjiaCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    back() {
      ...
      this.parent.contentStyleChange();
                                                                        ...
                                                                        tianjiatupianUploadChange(fileUrls) {
                this.ruleForm.tianjiatupian = fileUrls;
				this.addEditUploadStyleChange()
            },
                                                                        	addEditStyleChange() {
	  this.$nextTick(()=>{
